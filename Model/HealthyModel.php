...
class HealthyModel{
    ...
    public function get_calories($cal){
        ...
        $stmt = $cnx->prepare("SELECT * FROM cadre JOIN recette ON cadre.id_recette=recette.id_recette WHERE recette.nb_calories<$cal");
        $stmt->execute();
    ...
    public function get_healthy(){
        ...
        $stmt = $cnx->prepare("SELECT titre_cadre, desc_cadre, img_cadre FROM cadre 
        ...
        WHERE ingredient.healthy=TRUE GROUP BY recette.id_recette");
        $stmt->execute();
        //Ne pas laisser la cnx a la BDD etablie
        $this->db->deconnexion($cnx);
        return $stmt;
